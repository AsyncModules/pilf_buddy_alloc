...
计划设计一种专用的指针类型，用于头指针和链表节点指针。

`type LinkNode = MarkedPtr<PIPtr<AtomicPtr<()>>>`

`MarkedPtr`、`PIPtr`、`AtomicPtr<()>`均实现了一个trait，代表各种指针操作和CAS

```Rust
pub trait LinkedPtr {
    fn value(&self) -> *mut (); // 获得该对象的值，没有进行去标记和地址转换
    fn ptr(&self) -> *mut (); // 此处获取的也是该对象的值，且进行了去标记和地址转换，保证获取的值可以直接作为指针访问对应地址。
    fn from_value(value: *mut ()) -> Self;
    fn from_ptr(ptr: *mut ()) -> Self; // 此处value和ptr的含义区别同上。保留哪一个接口或两个都保留留待实现时决定。
    fn from_my_ptr(my_ptr: *mut ()) -> &mut Self; // my_ptr为指向该对象的指针，而非该对象的值。
    fn set(&mut self, value: *mut ());
    fn compare_exchange(&mut self, current: *mut (), new: *mut ()) -> Result<*mut (), *mut ()>; // 这里的参数视为value

    fn default() -> Self {
        return Self::from_value(EMPTY_FLAG);
    }
    fn eq(&self, other: &Self) -> bool {
        return self.value() == other.value();
    }
    fn next(&self) -> &mut Self {
        return Self::from_my_ptr(self.ptr());
    }
}
```

对`NodePtr`的操作大部分是按值操作（没有进行去标记和地址转换），只有少数情况下需要进行按指针操作（进行了去标记和地址转换）：
...
impl MarkedPtr {
    ...
    // mark和unmark不会改变原值，而是将新值作为返回值传递。
    fn mark(&self) -> *mut ();
    fn unmark(&self) -> *mut ();
}
```